...
static void read_elf_header(int fd, Elf_Ehdr* eh) {
    assert(lseek(fd, 0, SEEK_SET) == 0);
    assert(read(fd, (void*)eh, sizeof(Elf_Ehdr)) == sizeof(Elf_Ehdr));
...
static void display_elf_hedaer(Elf_Ehdr eh) {
...
static void read_section(int fd, Elf_Shdr sh, void* dst) {
    assert(dst != NULL);
    assert(lseek(fd, (off_t)sh.sh_offset, SEEK_SET) == (off_t)sh.sh_offset);
    assert(read(fd, dst, sh.sh_size) == sh.sh_size);
}

static void read_section_headers(int fd, Elf_Ehdr eh, Elf_Shdr* sh_tbl) {
...
static void display_section_headers(int fd, Elf_Ehdr eh, Elf_Shdr sh_tbl[]) {
    // warn: C99
...
static void read_symbol_table(int fd, Elf_Ehdr eh, Elf_Shdr sh_tbl[], int sym_idx) {
    Elf_Sym sym_tbl[sh_tbl[sym_idx].sh_size];
    ...
    int sym_count = (sh_tbl[sym_idx].sh_size / sizeof(Elf_Sym));
    // log
...
static void read_symbols(int fd, Elf_Ehdr eh, Elf_Shdr sh_tbl[]) {
    for (int i = 0; i < eh.e_shnum; i++) {
...
void parse_elf(const char* elf_file) {
    ...
    Elf_Shdr sh_tbl[eh.e_shentsize * eh.e_shnum];
